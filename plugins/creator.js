...
let name = await conn.getName(who)
 ...
 const ini = await conn.sendContactArray(m.chat, [
    [`${nomorown3}`, `${await conn.getName(nomorown3+'@s.whatsapp.net')}`, `👑 Developer Bot `, `🚫 Don't call me 🥺`, `⁨ kelaza89@gmail.com `, `🇮🇩 Indonesia`, `🚀 https://youtube.com/@xyzalbotz`, `Donasi lah bng`],
    [`${conn.user.jid.split('@')[0]}`, `${await conn.getName(conn.user.jid)}`, `🔥 Bot WhatsApp 🐣`, `📵 Don't spam/call me 😢`, `Nothing`, `🇮🇩 Indonesia`, `🚀 https://youtube.com/@xyzalbotz`, `🤖 Dont Forget To Donate!`]
  ], fkontak)
